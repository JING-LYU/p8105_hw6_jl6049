...
The dataset has No missing. But `pnumlbw` and `pnumsga` are all zero, so cancel those two columns. Convert several numeric variables to factor.
...
Based on the understanding, I `fincme` and `momage`and `menarche` are not relevant. So I try to figure this out by evaluating p values of those variables in a lm model. I find out `fincome` and `momage`and `menarche` is not significant then cancel it.
